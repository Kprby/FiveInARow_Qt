...
int AI::ValueOfStep(ChessStep step)
{
...
bool AI::GetLine(std::vector<int> &line, const ChessStep &step, const Direction &direction)
{
	...
	do
	{
		...
		++j;
	} while (j < 5 && chessPoint.IsInBounds() && (step.side == chess[chessPoint.x][chessPoint.y] || blank == chess[chessPoint.x][chessPoint.y]));

	//负方向边界判定
	if (!chessPoint.IsInBounds())
	{
		line.push_back(-1);
	}
	...
	//chessPoint走到正方向的尽头
	chessPoint = chessPoint + direction;
	while (j < 4 && chessPoint.IsInBounds() && (step.side == chess[chessPoint.x][chessPoint.y] || blank == chess[chessPoint.x][chessPoint.y]))
	{
		if (step.side == chess[chessPoint.x][chessPoint.y])
		{
			line.push_back(1);
...
void AI::GetLineTypeVector(std::vector<LineType> &lineTypeVector, const ChessStep &step)
{
...
void AI::GetLineType(LineType &primary, LineType &secondary, const std::vector<int> &line)
{
	//请保证line不为死棋
	if (line.size() < 7)
	{
...
void AI::GetLineType(LineType &lineType, const std::vector<int>::const_iterator &five)
{
	//five代表以迭代器为首连续5个棋
...
bool AI::MoreDense(const std::vector<int>::const_iterator &first, const std::vector<int>::const_iterator &second)
{
...
void AI::GetLiveSpace(const std::vector<int>::const_iterator &five, int &liveSpace, int &holdSpace)
{
...
int AI::ValueOfNoType(const ChessStep &step, const Direction &direction)
{
	...
	while (left.IsInBounds() && right.IsInBounds()
...
void AI::PrintLineType(const LineType &primary, const LineType &secondary)
{
...
void AI::PrintLineType(const LineType &l)
{
	if (l.type == -1)
...
void AI::PrintLineTypeVector(const std::vector<LineType> &lineTypeVector)
{
	for (const LineType &l : lineTypeVector)
	{
		PrintLineType(l);
...
AI::AI(const ChessType (&chess)[15][15], const ChessType side, const bool difficulty)
	: chess(chess), side(side), difficulty(difficulty)
{
